...
namespace PopulationSimulator
{
    public class BeneficialMutationRate
    {
        ...
        {
            int totalMutations = (from x in ObsData select x.MutCounter.CountOfEachMutation.Sum()).Sum();
            double totalTime = (from x in ObsData select x.PopSize.TotalGrowth*x.TotalTransfers).Sum();
            double totTime2 = (from x in ObsData select x.AmountOfTimeLastRun).Sum();
